...
- [ ] T002 [P] Inicializar `backend/package.json` (Express, TypeScript, driver de MongoDB o
  `mongoose`, `argon2`, `jsonwebtoken`, `@simplewebauthn/server`, `axios`, `zod` — validación de
  esquema FR-048, `helmet` — cabeceras de seguridad FR-047) y `backend/tsconfig.json`
...
- [ ] T028 [P] Test unitario de `handleRequest` (casos de éxito y error, `axios` mockeado) en
  `frontend/__tests__/__mocks__/handleRequest.test.ts` — escribir primero, debe fallar
- [ ] T029 [P] Implementar `handleRequest(method, endpoint, body?, headers?)` en
  `frontend/src/services/handleRequest.ts` (hace pasar T028)
- [ ] T030 [P] Implementar `LoadingProvider` (estado global de carga, se activa/desactiva
  alrededor de cada llamada de `handleRequest`) en `frontend/src/providers/LoadingProvider.tsx`
  (depende de T029)
- [ ] T031 [P] Implementar el overlay `Loader` circular a pantalla completa, fondo
...
- [ ] T032 [P] Implementar el componente `Button` compartido (fondo más claro en `:hover`) en
  `frontend/src/components/shared/Button.tsx`
- [ ] T033 [P] Implementar el componente `Input` compartido (label fuera y arriba, esquinas
  10px) en `frontend/src/components/shared/Input.tsx`
- [ ] T034 [P] Implementar los componentes `Card`/`Modal` compartidos en
  `frontend/src/components/shared/Card.tsx` y `frontend/src/components/shared/Modal.tsx`
- [ ] T035 Configurar el layout raíz con modo oscuro por defecto y `LoadingProvider` montado en
  `frontend/src/app/layout.tsx` (depende de T030, T031)
- [ ] T036 [P] Configurar el esqueleto de next-auth (estrategia `jwt`, sin providers todavía) en
...
- [ ] T037 [P] [US1] Contract test `POST /auth/register` (password y alta de passkey) en
  `backend/tests/contract/auth-register.test.ts`
- [ ] T038 [P] [US1] Contract test `POST /auth/login` (éxito, password incorrecto, método
  distinto al elegido, cuenta bloqueada) en `backend/tests/contract/auth-login.test.ts`
- [ ] T039 [P] [US1] Contract test `POST /auth/logout` en `backend/tests/contract/auth-logout.test.ts`
- [ ] T040 [P] [US1] Contract test de la ceremonia WebAuthn (registration-options/verify,
  authentication-options/verify) en `backend/tests/contract/auth-webauthn.test.ts`
- [ ] T041 [P] [US1] Contract test `GET/POST/DELETE /auth/passkeys` (incluye 409 al borrar la
  última) en `backend/tests/contract/auth-passkeys.test.ts`
- [ ] T042 [P] [US1] Integration test: 5 logins fallidos por contraseña → 423 bloqueado → el
  contador se resetea con éxito o al expirar los 15 min en
  `backend/tests/integration/auth-lockout.test.ts` (FR-036)
- [ ] T043 [P] [US1] Integration test: la sesión expira al día en
  `backend/tests/integration/auth-session-expiry.test.ts` (FR-037)
- [ ] T044 [P] [US1] Test de frontend: Login/Registro — validación, mensajes de error/reintento
...
- [ ] T045 [P] [US1] Test de frontend: Dashboard shell — renderiza las 4 tarjetas correctas y el
...
- [ ] T046 [P] [US1] Test de frontend: `PasskeyManager` bloquea borrar la última passkey
  (`handleRequest` mockeado) en `frontend/__tests__/auth/passkey-manager.test.tsx`

### Implementation for User Story 1

- [ ] T047 [P] [US1] Implementar la entidad de dominio `User` (inmutabilidad de `authMethod`,
  password ≥4 caracteres, FR-042) en `backend/src/modules/auth/domain/user.ts`
- [ ] T048 [P] [US1] Implementar la entidad de dominio `PasskeyCredential` en
  `backend/src/modules/auth/domain/passkeyCredential.ts`
- [ ] T049 [US1] Implementar `UserRepository` (Mongo) en
  `backend/src/modules/auth/infrastructure/userRepository.ts` (depende de T009, T047)
- [ ] T050 [US1] Implementar `PasskeyCredentialRepository` (Mongo) en
  `backend/src/modules/auth/infrastructure/passkeyRepository.ts` (depende de T009, T048)
- [ ] T051 [US1] Implementar el comando `RegisterUser` (FR-001, FR-042; hashea la contraseña con
  ...
  de T049; hace pasar T037)
- [ ] T052 [US1] Implementar el comando `LoginWithPassword` (bloqueo FR-036, rechazo de método
  distinto FR-002) en `backend/src/modules/auth/application/commands/loginWithPassword.ts`
  (depende de T049, T023; hace pasar T038, T042)
- [ ] T053 [US1] Implementar la ceremonia de alta de passkey (`@simplewebauthn/server`,
  FR-005/FR-006) en `backend/src/modules/auth/application/commands/registerPasskey.ts` (depende
  de T050; hace pasar parte de T040)
- [ ] T054 [US1] Implementar la ceremonia de login por passkey (FR-002) en
  `backend/src/modules/auth/application/commands/loginWithPasskey.ts` (depende de T050; hace
  pasar el resto de T040)
- [ ] T055 [US1] Implementar el comando `DeletePasskey` (bloquea la última, FR-007) en
  `backend/src/modules/auth/application/commands/deletePasskey.ts` (depende de T050; hace pasar
  T041)
- [ ] T056 [US1] Implementar el comando `Logout` (invalida la sesión de inmediato, FR-041) en
  `backend/src/modules/auth/application/commands/logout.ts` (depende de T017; hace pasar T039)
- [ ] T057 [P] [US1] Implementar la query `ListPasskeys` en
...
- [ ] T063 [P] [US1] Construir el shell del Dashboard (4 tarjetas horizontales responsive
  ...
  (hace pasar T045)
- [ ] T064 [P] [US1] Construir la gestión de passkeys (listado con nombre, alta, baja con
  bloqueo de la última) en `frontend/src/components/auth/PasskeyManager.tsx` (hace pasar T046)
...
- [ ] T065 [P] [US2] Contract test `GET/POST /money-sources` (incluye 409 duplicado, 400 >60
...
- [ ] T073 [P] [US2] Implementar entidad de dominio + repositorio `MoneySource` en
...
- [ ] T074 [P] [US2] Implementar entidad de dominio + repositorio `Category` en
  `backend/src/modules/categories/domain/category.ts` y
  `backend/src/modules/categories/infrastructure/categoryRepository.ts`
- [ ] T075 [P] [US2] Implementar entidad de dominio + repositorio `Transaction` (precisión de 2
...
- [ ] T076 [US2] Sembrar las fuentes (Santander, BNA, Macro, Lemon, Brubank, Efectivo) y
...
- [ ] T082 [US2] Implementar el comando `UpdateTransaction` (FR-018) en
  ...
  T075; hace pasar parte de T067)
- [ ] T083 [US2] Implementar el comando `DeleteTransaction` (FR-019) en
  `backend/src/modules/transactions/application/commands/deleteTransaction.ts` (depende de
  T075; hace pasar el resto de T067)
- [ ] T084 [US2] Implementar `backend/src/modules/money-sources/interface/moneySourceRoutes.ts`
  (solo `GET`/`POST`; `PUT`/`DELETE` responden 404, FR-015; usa T013 `validateSchema`, FR-048)
  (depende de T020, T025, T077, T078; hace pasar el resto de T065)
- [ ] T085 [US2] Implementar `backend/src/modules/categories/interface/categoryRoutes.ts` (mismo
...
- [ ] T088 [P] [US2] Construir la pantalla "Alta de categoría" (centrada, 2 columnas) en
  ...
  `frontend/src/components/categories/CategoryForm.tsx` (hace pasar parte de T070)
- [ ] T089 [P] [US2] Construir la pantalla "Alta de fuente de dinero" (centrada, 2 columnas) en
  `frontend/src/app/fuentes/nueva/page.tsx` y
  `frontend/src/components/money-sources/MoneySourceForm.tsx` (hace pasar el resto de T070)
- [ ] T090 [US2] Construir el shell de la pantalla "Transacciones" (grilla de 4 cuadrantes,
  apilado en columna <500px) en `frontend/src/app/transacciones/page.tsx`
- [ ] T091 [US2] Construir el formulario de alta de transacción (cuadrante superior izquierdo,
  botón deshabilitado durante el envío, FR-045) en
  `frontend/src/components/transactions/TransactionForm.tsx` (depende de T090; hace pasar T071)
- [ ] T092 [US2] Construir el historial/listado base (cuadrante inferior izquierdo de
  ...
  `frontend/src/components/transactions/TransactionHistory.tsx` (depende de T082, T083, T091;
  hace pasar T072). Los filtros por período y la paginación se agregan en US4 (T104) sobre este
  mismo componente.
...
- [ ] T093 [P] [US3] Contract test `GET /balances` (por fuente y consolidado) en
  `backend/tests/contract/balances.test.ts`
- [ ] T094 [P] [US3] Integration test: saldo = ingresos − egresos por fuente/moneda en
  `backend/tests/integration/balances-calculation.test.ts` (FR-021/FR-022, SC-007)
- [ ] T095 [P] [US3] Test de frontend: `BalanceTable` renderiza filas por fuente + totales
  consolidados (`handleRequest` mockeado) en `frontend/__tests__/balances/balance-table.test.tsx`

### Implementation for User Story 3

- [ ] T096 [US3] Implementar la query `GetBalances` (agregación Mongo por fuente+moneda y por
  moneda) en `backend/src/modules/balances/application/queries/getBalances.ts` (depende de T075;
  hace pasar T093, T094)
- [ ] T097 [US3] Implementar `backend/src/modules/balances/interface/balanceRoutes.ts` (usa T013
  `validateSchema`, FR-048) y montarla en `backend/src/app.ts` (depende de T020, T025, T096)
- [ ] T098 [US3] Construir la tabla de saldos por fuente (cuadrante superior derecho de
  "Transacciones") en `frontend/src/components/balances/BalanceTable.tsx` (depende de T090;
  hace pasar T095)
...
- [ ] T099 [P] [US4] Contract test `GET /transactions` con filtros día/mes/año y paginación en
  `backend/tests/contract/transactions-list.test.ts`
- [ ] T100 [P] [US4] Integration test: límite de paginación (50 vs. 51) sin repetir ni omitir en
  `backend/tests/integration/transactions-pagination.test.ts` (FR-024)
- [ ] T101 [P] [US4] Test de frontend: cambio de filtros día/mes/año y paginación sin repetidos
...
- [ ] T102 [US4] Implementar la query `ListTransactions` (filtro día/mes/año, tamaño de página
  ...
  (FR-023/FR-024; depende de T075; hace pasar T099, T100)
- [ ] T103 [US4] Agregar `GET /transactions` a
  `backend/src/modules/transactions/interface/transactionRoutes.ts` (depende de T086, T102)
- [ ] T104 [US4] Extender `TransactionHistory` (construida en US2, T092) con filtros por
  día/mes/año y controles de paginación, sin tocar la lógica de edición/borrado ya existente, en
  `frontend/src/components/transactions/TransactionHistory.tsx` (depende de T092, T102; hace
  pasar T101)
...
- [ ] T105 [P] [US5] Contract test `GET /charts/expenses-by-category` (mes por defecto, rango de
  fechas, filtro de categoría) en `backend/tests/contract/charts.test.ts`
- [ ] T106 [P] [US5] Integration test: la distribución porcentual respeta la regla de redondeo
...
- [ ] T107 [P] [US5] Test de frontend: vista por defecto del mes en curso + interacción con
  filtros de `ExpensesPieChart` (`handleRequest` mockeado) en
  `frontend/__tests__/charts/expenses-pie-chart.test.tsx`

### Implementation for User Story 5

- [ ] T108 [US5] Implementar la query `GetExpensesByCategory` (mes en curso calculado en zona
  ...
  FR-027; depende de T075; hace pasar T105, T106)
- [ ] T109 [US5] Implementar `backend/src/modules/charts/interface/chartRoutes.ts` (usa T013
  `validateSchema`, FR-048) y montarla en `backend/src/app.ts` (depende de T020, T025, T108)
- [ ] T110 [US5] Construir el gráfico de torta con `recharts` (cuadrante inferior derecho de
  "Transacciones", filtros de fecha/categoría) en
  `frontend/src/components/charts/ExpensesPieChart.tsx` (depende de T090; hace pasar T107)
...
- [ ] T111 [P] [US6] Contract test `GET /converter/rates` + `POST /converter/convert` (incluye
...
- [ ] T112 [P] [US6] Integration test: timeout de 5000ms a dolarapi.com (mockeado con `nock`)
  produce error explícito sin valor de conversión en
  `backend/tests/integration/converter-timeout.test.ts` (FR-032, SC-004)
- [ ] T113 [P] [US6] Test de frontend: error explícito y sin valor de conversión ante fallo de
  la fuente (`handleRequest` mockeado con 502) en
  `frontend/__tests__/converter/converter-form.test.tsx` (FR-032, RF30)

### Implementation for User Story 6

- [ ] T114 [US6] Implementar el cliente de dolarapi.com con timeout de 5000ms en
  ...
  hace pasar T112)
- [ ] T115 [US6] Implementar la query `GetRates` (mapea los 7 tipos a la `casa` de dolarapi.com,
  FR-029) en `backend/src/modules/converter/application/queries/getRates.ts` (depende de T114)
- [ ] T116 [US6] Implementar el comando `ConvertAmount` (usa `venta`, FR-030/FR-031) en
  `backend/src/modules/converter/application/commands/convertAmount.ts` (depende de T114)
...
- [ ] T122 [P] Verificar el presupuesto de carga de SC-003 (<2s a 10 Mbps) en las rutas
  principales
- [ ] T123 [P] Contract test de regresión: confirmar que no existe ninguna ruta pública para
...
- [ ] T124 [P] Contract test de regresión: toda respuesta HTTP incluye el set base de cabeceras
...
- [ ] T125 [P] Contract test de regresión: un `body`/`query` con forma inválida (tipo incorrecto,
...
- [ ] T126 Revisión de seguridad: confirmar que no hay secretos commiteados y que los
  `.env.example` reflejan las variables de research.md (Principio IV)
- [ ] T127 [P] Documentar y verificar la configuración de cifrado en reposo de MongoDB (motor de
  almacenamiento cifrado o cifrado gestionado del proveedor elegido en T009) en
  `backend/README.md` (FR-034)
- [ ] T128 [P] Actualizar instrucciones de ejecución en `AGENTS.md`/README si se agregaron
  scripts nuevos
...
- **Polish (Phase 9)**: depende de las historias que se quieran incluir en el release; T123-T125
...
- **US2 (P1)**: reutiliza `RegisterUser` de US1 para sembrar fuentes/categorías (T076); es
  independientemente testeable con las fuentes/categorías predefinidas. Construye
  `TransactionHistory` (T092) con edición y borrado incluidos — esto es deliberado: FR-018/
...
- **US4 (P2)**: **depende directamente del componente `TransactionHistory` construido en US2**
  (T092) — T104 lo extiende en vez de crear un componente nuevo, para no duplicar la lógica de
...
- Una vez completada Foundational, US1 y US6 pueden desarrollarse en paralelo de inmediato; US3
  ...
  "Transacciones" (T090, US2); US4 no puede empezar su tarea de frontend (T104) hasta que T092
  (US2) exista
...
de fábrica —T076— requiere que `RegisterUser` de US1 exista, y US4 no puede tocar su tarea de
...
- Evitar: tareas vagas, conflictos de mismo archivo entre tareas [P], dependencias cruzadas
  ...
  US4 → T092 de US2, documentada arriba)
